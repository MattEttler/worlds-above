...
def initialize_lighting_surface(
    ...
    for i in range(diffuse_scaled):
        for j in range(diffuse_scaled):
            distance = N.sqrt(
                    (i-(diffuse_scaled//2))**2
                    + (j-(diffuse_scaled//2))**2
                    )
            intensity = int(distance
                            / (N.sqrt((diffuse_scaled//2)**2
                                      + (diffuse_scaled//2)**2)) * 255
                            )
...
def render_lights(screen, x_scale_ratio: int, y_scale_ratio: int):
    blits_sequence = tuple([(
        filter_surf, (
            int(
                (bounding_boxes[entity].x*x_scale_ratio)
                - (filter_surf.get_width()/2)
                ),
            int(
                (bounding_boxes[entity].y*y_scale_ratio)
    ...
    whole_filter = pygame.surface.Surface(
